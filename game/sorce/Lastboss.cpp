...
bool	LastBoss::Process()
{
	...
	case STATUS::STAB:
		...
		if (actionFlag == true)
		{
			...
			if (attackStep == 2)
			{
				if (_modelInf.playTime > 5) { Move(90.0f * awakeDmg, .0f); }
			}
			if (attackStep == 3) {
				if (_modelInf.playTime > 5) { Move(30.0f * awakeDmg, .0f); }
			}
			break;
		}
		if (attackStep == 1) 
