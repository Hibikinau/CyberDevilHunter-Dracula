...
{/*
	...
	for (int i = 0; i < _modelInf.wepons.size(); i++)
	{
		if (_modelInf.wepons[i].isActive)
		{
			...
			attackColl acoll;
			acoll.capColl.underPos = MV1GetFramePosition(_modelInf.modelHandle, MV1SearchFrame(_modelInf.modelHandle, "右人指１"));//VTransform(weponPos, _modelInf.wepons[i].weponFrameMatrix);
